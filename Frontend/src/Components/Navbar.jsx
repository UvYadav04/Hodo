...
export default function Navbar() {
  ...
  const handlesearch = async (value) => {
    ...
    if (value !== "") {
      ...
      const response = await fetch("https://hodobackend.onrender.com0/search/userdata", {
        method: "POST",
        headers: {
              ...
              data.map((item) => {
                return (
                  <div className='d-flex dipdop flex-row justify-content-start bg bg-white text-black mb-1 w-md-auto w-100 rounded-3s' onClick={() => navigate('/usersprofile', { state: item[0].Username })} >
                    <div className="image">
                      {item[0].image !== "" ? <img src={`https://hodobackend.onrender.com0//Images/` + item[0].image} width={40} height={40} className='d-inline rounded-5 my-1 mx-2' alt="" /> : <img src={usericon} width={40} height={40} className='d-inline rounded-5 my-1 mx-2' alt="" />}
                    </div>
                    <div className="info">
