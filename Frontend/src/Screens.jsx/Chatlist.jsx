        ...
        <div className="chatlist pb-0">
            ...
            <div className="friendslist">
                ...
                <ul className={!loading && !nofrnds ? "p-1 mt-4 friendsul" : "d-none"}>
                    {
                        Friends.map((item) => {
                            return (<li key={item} className={actives.includes(item) ? "text-dark cursor-pointer ms-1 my-2 p-1 bg bg-white rounded-2 px-2 d-flex flex-row active" : "text-dark cursor-pointer ms-1 my-2 p-1 bg bg-white rounded-2 px-2 d-flex flex-row"} onClick={() => navigate('/chat', { state: item.Username })}>
